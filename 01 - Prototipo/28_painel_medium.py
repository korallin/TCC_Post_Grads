...
def update_figure(vara_selecionada,ano_escolhido):
    ...
    df_janeiro = vara_escolhida[vara_escolhida['mes_primeira_dist']=='01 - Jan']
    total_janeiro = len(df_janeiro.index)
    df_fevereiro = vara_escolhida[vara_escolhida['mes_primeira_dist']=='02 - Fev']
    total_fevereiro = len(df_fevereiro.index)
    df_marco = vara_escolhida[vara_escolhida['mes_primeira_dist']=='03 - Mar']
    total_marco = len(df_marco.index)

    #vara_escolhida['Assunto'] = vara['Assunto'].value_counts()[:5].index.tolist()

    traces = []

    for assunto in df_janeiro['Assunto']:
        vara_filtrada_jan = df_janeiro[df_janeiro['Assunto']==assunto]
        traces.append(go.Bar(
                       x=["Janeiro"],
                       y=vara_filtrada_jan['Assunto'],
                       showlegend=False,
                       text=total_janeiro,
                       textposition='auto'
        ))

    for assunto in df_fevereiro['Assunto']:
        vara_filtrada_fev = df_fevereiro[df_fevereiro['Assunto']==assunto]
        traces.append(go.Bar(
                       x=["Fevereiro"],
                       y=vara_filtrada_fev['Assunto'],
                       showlegend=False,
                       text=total_fevereiro,
                       textposition='auto'
        ))

    for assunto in df_marco['Assunto']:
        vara_filtrada_mar = df_marco[df_marco['Assunto']==assunto]
        traces.append(go.Bar(
                       x=["Março"],
                       y=vara_filtrada_mar['Assunto'],
                       showlegend=False,
                       text=total_marco,
                       textposition='auto'
        ))
